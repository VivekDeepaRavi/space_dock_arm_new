...
# Define constants
KNOWN_WIDTH = 11.0  # The actual width of the object in cm
    ...
    # Capture each frame
        ...
        if np.all(ids is not None):
            if 0 in ids and 1 in ids:
                ...
                position = (center_x - frame.shape[1] / 2) * distance / FOCAL_LENGTH
                ...
                if abs(position) <= POSITION_LIMIT:
                    ...
                    cv2.putText(frame, f'Position (cm): {position:.2f}', (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
                    cv2.putText(frame, f'Distance (cm): {distance:.2f}', (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
                    hello_str = position
                    rospy.loginfo(hello_str)
                else:
                    hello_str = 5000
                    rospy.loginfo(hello_str)
                    cv2.putText(frame, 'Object out of view', (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
