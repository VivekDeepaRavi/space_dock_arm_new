...
sleep 5
/usr/bin/tmux -2 new-session -d -s arm_controller
/usr/bin/tmux send-keys -t arm_controller.0 "roslaunch dynamixel_workbench_controllers one_r.launch" ENTER
...
sleep 5
/usr/bin/tmux -2 new-session -d -s arduino_controllers
/usr/bin/tmux send-keys -t arduino_controllers.0 "rosrun rosserial_arduino serial_node.py /dev/ttyACM0" ENTER
...
/usr/bin/tmux send-keys -t brave.0 "brave-browser --kiosk file:///home/asimov/space_dock_arm/src/space_arm_ui-master/arm.html" ENTER   #change the browser and path according to your system.
